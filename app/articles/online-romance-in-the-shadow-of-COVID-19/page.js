...
export async function generateMetadata() {
  const article = contentWritingArticles.find(article => article.slug === 'online-romance-in-the-shadow-of-COVID-19');
  ...
  return {
    ...
    twitter: {
      ...
      creator: '@urmic660', // Replace with actual Twitter handle
    },
    robots: {
      index: true,
      follow: true,
...
export default async function OnlineRomanceEditingSamplePage() {
  const article = contentWritingArticles.find(article => article.slug === 'online-romance-in-the-shadow-of-COVID-19');
  ...
  const { default: OnlineRomanceClient } = await import('./OnlineRomanceClient');

  return (
    <>
      <StructuredData article={article} />
      <OnlineRomanceClient article={article} />
    </>
  );
} 